...
module.exports = (client) => {
  ...
  function receiveQueueData(newQueue, newTrack) {
    ...
    var embed = new MessageEmbed().setColor(ee.color)
      ...
      .addField(`❔ Instalar a música:`, `>>> [\`Clique aqui\`](${newTrack.streamURL})`, true)
      .addField(`❔ Filros${newQueue.filters.length > 0 ? "s": ""}:`, `>>> ${newQueue.filters && newQueue.filters.length > 0 ? `${newQueue.filters.map(f=>`\`${f}\``).join(`, `)}` : `${client.allEmojis.x}`}`, newQueue.filters.length > 1 ? false : true)
